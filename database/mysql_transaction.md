...
### 1按性能分：乐观锁(用版本比对实现)和悲观锁

- 乐观锁：不加锁，通过版本比对，不符合时执行失败，失败后可以重试；

- 悲观锁：

### 2 按操作分：读锁/共享锁Shared Lock 和 写锁/排他锁 Exclusive Lock
...
### 3 按操作粒度： 行锁Record Lock 和 表锁Table Lock

- 行锁只锁定当前数据行，锁的粒度小，加锁慢，发生锁冲突的概率小，并发度高；

- 表锁则锁定整个表，锁的粒度大，加锁快，开销小，但是锁冲突的概率大，并发度低；（一般用在整表数据迁移的情况）
