...
问题：事务A事先读取了数据，事务B紧接着更新数据并未提交，而事务A再次读取该数据时，数据已经发生了改变。这就造成了脏读；
...
问题：事务A事先读取了数据，事务B紧接着更新数据并提交事务，而事务A再次读取该数据时，数据已经发生了改变(主要针对update操作)。这就造成了不可重复读（虚读）。

### 3 可重复读 REPEATABLE READ 

解决了不可重复读(虚读)；

问题:事务A读取与搜索条件相匹配的若干行；事务B以插入或删除行等方式来修改事务A的结果集，然后再提交；事务A再读取时，却发现数据发生了变化（主要是insert/delete操作）；这就造成了幻读。

### 4 串行化 SERIALIZABLE

串行化，即所有的事务串行执行，一个事务提交后另一个事务才能执行操作；
